...
class Tile(Listenable):
    ...
    def __repr__(self):
        return f"Tile({self.row}, {self.col}, {self.value.__repr__()})"
...
class Board(object):
        ...
        for group in self.groups:
            ...
            for value in leftovers:
                ...
                for index in range(len(group)):
                    if value in group[index].candidates:
                        count += 1
                        index_val = index
                if count == 1:
                    group[index_val].set_value(value)
                    val = True
        return val
    ...
    def solve(self):
        ...
        if self.is_complete():
            ...
            tile_guess = self.min_choice_tile()
            for tile in tile_guess.candidates:
                tile_guess.set_value(tile)
                if self.solve():
                    return True
                else:
    ...
    def min_choice_tile(self) -> Tile:
        ...
        for tile in self.tiles:
            for val in tile:
                if val.value == UNKNOWN and len(val.candidates) < len(min_tile.candidates):
                    min_tile = val
        return min_tile
